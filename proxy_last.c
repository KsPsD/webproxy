...
/*여기서 부터 read write 설명 
queue가 왜 하나 더 생겼나. 전체 흐름으로 봤을때 reader 나 writer 둘 중 하나를 우선시 해서 락을 걸게되면 
starvation한 상황이 일어 날 수 밖에 없음 따라서 queue라는 세마포를 둬서 reader나 writer 어떤 쪽이든 갯수가 자꾸
늘어나면 자원을 계속 점유하지 않게 하려는 목적임

따라서 queue를 reader가 얻고 일단 cache접근 뮤텍스를 락한뒤 critical section에 들어가기 전에 queue에 대한 락을 풀어주는데
다른 쓰레드에서 이 상황에서 writer에 접근 할 수가 있는데 writer는 queue가 풀렸으니 queue에대해 락을 걸 것이다.
하지만 쓰레드 접근에 대한 세마포를 reader가 가지고 있으니 writer는 대기 할 것이다. 그 뒤 reader가 캐쉬 접근이 
끝난 다음에 캐쉬 접근 세마포를 반납하면 writer가 가져갈 수 있는 상황이 나올 수 있다. 이런 과정으로 자연스레 
돌아가면서 자원을 점유 하게 된다.
