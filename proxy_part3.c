...
int main(int argc, char **argv)
{
    ...
    Signal(SIGPIPE, SIG_IGN); //연결 끊어져도 SIGPIPE 시그널 안뜨게 무시 //SIGPIPE 무시; 네트웍 자료전송중 클라이언트가 끊기는 경우 프로그램이 종료되는 것을 막기 위함
...
void *thread(void *vargp)
{
    int connfd = (int)vargp; //이렇게 안받으면 race 상황때문에 어떻게 될지 모름
...
//여기서는 reader우선임
void readerPre(int i) //처음 읽을때
{
    P(&cache.cacheobjs[i].rdcntmutex); //readcnt 접근 가능하게 하는 mutex
    cache.cacheobjs[i].readCnt++;
    if (cache.cacheobjs[i].readCnt == 1) //처음 reader일때 캐쉬 접근함
        P(&cache.cacheobjs[i].wmutex);
    V(&cache.cacheobjs[i].rdcntmutex);
}

void readerAfter(int i) //다읽은 다음에
{
...
//write는 reader가 끝날때 까지 기다려야됨 reader가 계속 생겨나면 writer기다려야만 할 수도 있음 근데 제한 있어서 ㄱㅊ할듯
void writePre(int i)
{
    P(&cache.cacheobjs[i].wmutex);
}
